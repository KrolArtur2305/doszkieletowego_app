...
export default function BudzetScreen() {
  ...
  const loadBudget = useCallback(async () => {
    ...
    try {
      ...
      const completedStatuses = new Set(['zrealizowany', 'wykonany', 'done', 'completed', 'ukoñczony']);
  ...
  const removeReceiptFileEverywhere = useCallback(async (receiptPath: string, documentPath = receiptPath) => {
    ...
    await Promise.all(targets.map(async ({ bucket, path }) => {
      ...
      if (error && !String(error.message || '').includes('not found')) {
        console.warn(`[Bud¿et] nie uda³o siê usun¹æ pliku z bucketu ${bucket}:`, error.message);
      }
    }));
  }, []);
  ...
  const syncLinkedReceiptDocument = useCallback(async ({
    ...
    if (!filePath) {
      if (previousPath && previousPath !== filePath) {
        ...
        if (deleteError) {
          console.warn('[Bud¿et] nie uda³o siê usun¹æ powi¹zanego dokumentu:', deleteError.message);
        }
      }
      return;
    }
    ...
    if (previousPath && previousPath !== filePath) {
      const { error: deleteError } = await supabase
        ...
        .eq('plik_url', previousPath);
      if (deleteError) {
        console.warn('[Bud¿et] nie uda³o siê usun¹æ starego dokumentu po zmianie pliku:', deleteError.message);
      }
    }
    ...
    try {
      ...
      if (receiptFilePath) {
        try {
          ...
          console.warn('[Bud¿et] nie uda³o siê zsynchronizowaæ dokumentu wydatku:', docError?.message || docError);
          Alert.alert(
            t('errorTitle'),
